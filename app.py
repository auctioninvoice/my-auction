...
st.markdown("""
    ...
    .total-highlight { background-color: #e9ecef; padding: 10px; border-radius: 5px; text-align: right; font-weight: bold; font-size: 1.2em; color: #212529; margin-bottom: 10px; border-right: 5px solid #6c757d; }
    </style>
        ...
        if selected_person == "SUMMARY_MODE":
            ...
            if not filtered_df.empty:
                ...
                for p in all_p:
                    ...
                    total_buy_fees += b_f
                    bal = s_net - (b_raw + b_f)
                    if bal > 0: pay_out.append({"고객명": p, "정산금액": bal})
                    elif bal < 0: pay_in.append({"고객명": p, "입금금액": abs(bal)})
                ...
                with col_in:
                    ...
                    total_in_remain = 0
                    current_in_items = []
                    for item in sorted(pay_in, key=lambda x: x['입금금액'], reverse=True):
                        c_check, c_name, c_amt = st.columns([1, 4, 4])
                        is_checked = c_check.checkbox("", key=f"in_{selected_date}_{item['고객명']}")
                        c_name.markdown(f"**{item['고객명']}**")
                        c_amt.markdown(f"{item['입금금액']:,.0f}원")
                        if not is_checked: total_in_remain += item['입금금액']
                    in_container.markdown(f"<div class='total-highlight'>남은 미입금 합계: {total_in_remain:,.0f}원</div>", unsafe_allow_html=True)

                with col_out:
                    st.subheader("💵 정산 드릴 돈 (판매자)")
                    out_container = st.container()
                    total_out_remain = 0
                    for item in sorted(pay_out, key=lambda x: x['정산금액'], reverse=True):
                        c_check, c_name, c_amt = st.columns([1, 4, 4])
                        is_checked = c_check.checkbox("", key=f"out_{selected_date}_{item['고객명']}")
                        c_name.markdown(f"**{item['고객명']}**")
                        c_amt.markdown(f"{item['정산금액']:,.0f}원")
                        if not is_checked: total_out_remain += item['정산금액']
                    out_container.markdown(f"<div class='total-highlight'>남은 미정산 합계: {total_out_remain:,.0f}원</div>", unsafe_allow_html=True)

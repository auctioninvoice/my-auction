        ...
        if selected_person == "SUMMARY_MODE":
            ...
            if not filtered_df.empty:
                ...
                with col_in:
                    ...
                    total_in_remain = 0
                    for i, item in enumerate(sorted(pay_in, key=lambda x: x['금액'], reverse=True)):
                        c_chk, c_name, c_amt = st.columns([1, 4, 4])
                        checked = c_chk.checkbox("", key=f"in_{selected_date}_{item['고객명']}")
                        c_name.markdown(f"**{item['고객명']}**")
                        c_amt.markdown(f"{item['금액']:,.0f}원")
                        if not checked: total_in_remain += item['금액']
                ...
                with col_out:
                    ...
                    total_out_remain = 0
                    for i, item in enumerate(sorted(pay_out, key=lambda x: x['금액'], reverse=True)):
                        c_chk, c_name, c_amt = st.columns([1, 4, 4])
                        checked = c_chk.checkbox("", key=f"out_{selected_date}_{item['고객명']}")
                        c_name.markdown(f"**{item['고객명']}**")
                        c_amt.markdown(f"{item['금액']:,.0f}원")
                        if not checked: total_out_remain += item['금액']
